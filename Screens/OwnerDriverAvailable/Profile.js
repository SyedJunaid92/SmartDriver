...
const Profile = () => {
  ...
  const UpdateData = async () => {
    CONSTANT.API.put('/carOwner/update', data).then(response => {
      if (response.data.code == 0) {
        alert('Details Updated Succesfully');
        const jsonValue = JSON.stringify(data);
        AsyncStorage.setItem('OwnerData', jsonValue);
      ...
      <View style={{flex: 2, padding: 10}}>
        {data != undefined && (
          <ScrollView>
            ...
            />
            <Text style={styles.ProfileTitleText}>Car Title</Text>
            <TextInput
              style={{
                borderWidth: 0.5,
                borderRadius: 10,
            ...
            />
            <Text style={styles.ProfileTitleText}>Car Registration City</Text>
            <TextInput
              style={{
                borderWidth: 0.5,
                borderRadius: 10,
